...
pub fn run(query: Option<&str>, as_json: bool) {
    ...
    if let Some(q) = query {
        ...
        let matched: Vec<&NutrientInfo> = nutrients
            .iter()
            .filter(|n| {
        ...
        if as_json {
            println!(
                "{}",
                serde_json::to_string_pretty(&nutrients).unwrap_or_default()
            );
            return;
        }
        ...
        println!(
            "{}",
            "🥗 Nutritional Deficiency Reference".bright_green().bold()
        );
        ...
        println!(
            "{}",
            "   Example: `openhealth nutrition fatigue` to find deficiencies causing fatigue."
                .dimmed()
        );
    }
}
...
fn print_nutrient_detail(n: &NutrientInfo) {
    ...
    println!(
        "     Food sources: {}",
        n.food_sources.join(", ").bright_green()
    );
...
pub fn assess(symptoms: &str, as_json: bool) {
    ...
    for n in &nutrients {
        let matched: Vec<&str> = n
            ...
            .filter(|ds| {
                ...
                input
                    .iter()
                    .any(|inp| ds_lower.contains(inp) || inp.contains(&ds_lower))
            })
            .copied()
            .collect();
    ...
    if matches.is_empty() {
        println!(
            "{} No nutritional deficiencies matched your symptoms.",
            "✗".red()
        );
    ...
    println!(
        "{}",
        "🥗 Nutritional Deficiency Assessment".bright_green().bold()
    );
    println!("{}", "━".repeat(60).bright_green());
    ...
    for m in &matches {
        ...
        println!(
            "  {} {} — {}% symptom match",
            indicator,
            m.nutrient.bright_yellow().bold(),
            pct
        );
        println!(
            "     Matched: {}",
            m.matched_symptoms.join(", ").bright_white()
        );
        println!("     Could indicate: {}", m.deficiency_disease.bright_red());
        println!(
            "     Eat more: {}",
            m.food_sources.join(", ").bright_green()
        );
        println!();
    }
    ...
    println!(
        "{}",
        "⚠️  This is informational only. See a healthcare provider for proper testing.".yellow()
    );
}
...
mod tests {
    ...
    fn test_all_nutrients_have_deficiency_symptoms() {
        for n in get_nutrients() {
            assert!(
                !n.deficiency_symptoms.is_empty(),
                "{} has no deficiency symptoms",
                n.nutrient
            );
        }
    }
    ...
    fn test_nutrients_count() {
        assert!(
            get_nutrients().len() >= 12,
            "Should have at least 12 nutrients"
        );
    }
}
